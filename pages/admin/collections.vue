      ...
      <div class="-mx-4 sm:-mx-8 px-4 sm:px-8 py-4 overflow-x-auto">
        <div class="inline-block min-w-full border-l border-r overflow-hidden">
          <table class="min-w-full leading-normal table-fixed">
            ...
            <tbody>
              <tr v-for="col in collections" :key="col.id" :class="{'bg-red-100': col.status === 'declined'}">
...
export default {
  ...
  methods: {
    async previewCollection(collection){
      this.isLoading = true
      const collectionData = await this.$store.dispatch('designer/fetchDesignData', collection.id)
