...
export default {
  ...
  methods: {
    ...
    increment () {
      if (!this.readonly){
        this.numericValue = this.updateValue(this.toNumber(this.numericValue) + this.step)
     ...
     */
    inputHandler (val) {
      this.numericValue = parseFloat(val)
      this.$emit('input', this.numericValue)
    },
    /**
    ...
    onBlur (event) {
      ...
      this.numericValue = this.updateValue(this.toNumber(value), value)
